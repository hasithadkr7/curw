...
class CsvToDssOperator(BaseOperator):
    def __init__(self, *args, **kwargs):
        super(CsvToDssOperator, self).__init__(*args, **kwargs)

    def execute(self, context):
        print 'executing {}'.__str__()


def csv_to_dss_convert():
    ...
    if start_time_ts:
        start_date_time_ts = datetime.datetime.strptime('%s %s' % (start_date_ts, start_time_ts), '%Y-%m-%d %H:%M:%S')
